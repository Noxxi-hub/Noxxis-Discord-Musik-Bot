...
async def create_controller_embed(bot, guild_id):
    ...
    if not is_playing:
        ...
        embed.set_image(url="attachment://nowplaying_bg.jpeg")
    ...
    # Leerzeile als Trenner zwischen Queue-Liste und Next Up
    if description:
        description += "\n\u200b"
    embed = Embed(
        title=get_messages("controller.title", guild_id),
    ...
    # Spacer between Next Up and Now Playing
    ...
    # Requester-Name: display_name (mit Emojis) statt Mention
    _req_name = getattr(requester, 'display_name', None) or getattr(requester, 'name', None) or str(requester)
    now_playing_value = get_messages(
        "controller.now_playing.value",
        guild_id,
        now_playing_title_display=now_playing_title_display,
        artist=artist,
        requester_name=_req_name,
    ...
    # Spacer between Now Playing and Status
    if status_lines:
        embed.add_field(name="\u200b", value="\u200b", inline=False)
        embed.add_field(
...
async def update_controller(
    ...
    Strategy: Always edit the existing controller if possible, never spam new messages.
    Auto-deletes the controller after 60s of inactivity (no music playing).
    """
    state = get_guild_state(guild_id)
    ...
    try:
        ...
        music_player = state.music_player
        is_playing = music_player.voice_client and music_player.current_info
        attach_file = None
        if not is_playing:
            bg_path = PROJECT_ROOT / "assets" / "nowplaying_bg.jpeg"
            if bg_path.exists():
                attach_file = discord.File(str(bg_path), filename="nowplaying_bg.jpeg")

        kwargs = {"content": None, "embed": embed, "view": view}
        if attach_file:
            kwargs["attachments"] = [attach_file]

        # ── Cancel any pending auto-delete timer for this guild ──
        if state.controller_delete_timer and not state.controller_delete_timer.done():
            state.controller_delete_timer.cancel()
            state.controller_delete_timer = None

        # ── Try to edit existing controller first ──
        ...
        if not edited:
            if interaction:
                ...
                state.controller_message_id = new_message.id
            else:
                kwargs["silent"] = True
                new_message = await channel.send(**kwargs)
                state.controller_message_id = new_message.id

        # ── Auto-delete after 60s if nothing is playing ──
        if not is_playing:
            state.controller_delete_timer = asyncio.create_task(
                _auto_delete_controller(bot, guild_id)
            )
...
async def _auto_delete_controller(bot, guild_id: int):
    """Deletes the controller message after 60 seconds of inactivity."""
    ...
    try:
        ...
        if channel:
            ...
            state.controller_message_id = None
            logger.info(f"[{guild_id}] Controller auto-deleted after 60s inactivity.")
    except (discord.NotFound, discord.Forbidden):
        state.controller_message_id = None
    except Exception as e:
        logger.error(f"[{guild_id}] Error auto-deleting controller: {e}")
