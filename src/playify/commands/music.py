...
# ==============================================================================
# Favoriten Commands — separate Subcommands wie im alten MusicBot
# ==============================================================================


@bot.tree.command(name="favorite", description="Favoriten verwalten")
@app_commands.describe(
    action="Aktion auswählen",
    name="Name des Favoriten",
    url="URL des Songs",
)
async def favorite(interaction: discord.Interaction, action: str = None, name: str = None, url: str = None):
    """Favoriten verwalten — Legacy-Kompatibilität."""
    # Neue Subcommands haben Vorrang, dieser Fallback bleibt für alte Aufrufe
    await interaction.response.send_message(
        "Nutze die neuen Commands: `/favorite_add`, `/favorite_list`, `/favorite_play`, `/favorite_remove`",
        ephemeral=True, silent=True,
    )


# ── /favorite_add ──────────────────────────────────────────────────────────────
@bot.tree.command(name="favorite_add", description="Song zu deinen Favoriten hinzufügen")
@app_commands.describe(
    url="URL des Songs (YouTube, Spotify etc.) — oder nichts für das aktuelle Lied",
)
async def favorite_add(interaction: discord.Interaction, url: str = None):
    """Song zu Favoriten hinzufügen. Ohne URL = aktueller Song."""
    if not interaction.guild:
        await interaction.response.send_message("Nur in Servern nutzbar.", ephemeral=True, silent=True)
        return
    ...
    # Wenn kein URL gegeben, aktuellen Song nehmen
    if not url:
        if not music_player.current_info:
            await interaction.response.send_message(
                "❌ Es läuft gerade kein Song. Gib einen Link an oder spiele zuerst etwas ab.",
                ephemeral=True, silent=True,
            )
            return
        title = music_player.current_info.get("title", "Unknown")
        track_url = music_player.current_info.get("webpage_url") or music_player.current_info.get("url", "")
        display_title = title
    else:
        # URL auflösen
        display_title = url
        ...
        try:
            info = await fetch_video_info_with_retry(
                url, ydl_opts_override={"extract_flat": True, "noplaylist": True}
            )
            if info:
                display_title = info.get("title", url)
    ...
    favs = list_favorites(guild_id)
    normalize = lambda u: (u or "").replace("https://www.", "https://").replace("http://www.", "http://").rstrip("/").strip()
    if any(normalize(f["url"]) == normalize(track_url) for f in favs):
        await interaction.response.send_message(
            f"⭐ **{display_title}** ist schon in deinen Favoriten!", ephemeral=True, silent=True
        )
        return

    if len(favs) >= 50:
        await interaction.response.send_message(
            "❌ Maximal 50 Favoriten. Erst einen löschen mit `/favorite_remove`.",
            ephemeral=True, silent=True,
        )
        return

    # Name = Titel (kann später umbenannt werden)
    ok = add_favorite(guild_id, title[:50], track_url, display_title, interaction.user.id)
    if ok:
        await interaction.response.send_message(
            f"⭐ Favorit **{title[:50]}** gespeichert! ({len(favs)+1}/50)",
            ephemeral=True, silent=True,
        )
    else:
        await interaction.response.send_message(
            f"❌ Favorit **{title[:50]}** existiert bereits.", ephemeral=True, silent=True,
        )


# ── /favorite_list ─────────────────────────────────────────────────────────────
@bot.tree.command(name="favorite_list", description="Deine gespeicherten Favoriten anzeigen")
async def favorite_list(interaction: discord.Interaction):
    """Alle Favoriten als anklickbare Buttons anzeigen."""
    if not interaction.guild:
        await interaction.response.send_message("Nur in Servern nutzbar.", ephemeral=True, silent=True)
        return
    ...
    if not favs:
        await interaction.response.send_message(
            "📭 Keine Favoriten gespeichert. Mit `/favorite_add` hinzufügen.",
            ephemeral=True, silent=True,
        )
        return

    view = FavoriteSelectView(favs, guild_id, interaction.user.id)

    embed = Embed(
        title=f"⭐ Favoriten ({len(favs)}/50)",
        description="Klick auf einen Song um ihn abzuspielen:",
        color=discord.Color.gold(),
    )

    # Zeige die ersten 20 als Buttons (Discord-Limit: 5 Reihen à 5)
    for i, f in enumerate(favs[:20]):
        title_short = (f.get("title") or f.get("url", "Unknown"))[:70]
        view.add_item(FavoriteButton(i, title_short, f["url"], f.get("title") or f["url"]))
...
class FavoriteButton(discord.ui.Button):
    """Ein anklickbarer Button für einen Favoriten."""
    def __init__(self, index: int, label: str, url: str, full_title: str):
        super().__init__(
            label=f"{index+1}. {label}"[:80],
            style=ButtonStyle.secondary,
            custom_id=f"fav_btn_{index}_{url[:50]}",
        )
        self.track_url = url
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.followup.send(
            f"⭐ **{self.full_title[:60]}** wird abgespielt!",
            silent=True,
        )


class FavoriteSelectView(discord.ui.View):
    """View für die Favorite-Liste mit Buttons."""
    def __init__(self, favs, guild_id, user_id):
...
# ── /favorite_play ─────────────────────────────────────────────────────────────
@bot.tree.command(name="favorite_play", description="Einen Favoriten abspielen")
@app_commands.describe(
    number="Nummer des Favoriten (aus /favorite_list)",
)
async def favorite_play(interaction: discord.Interaction, number: int):
    """Favorit per Nummer abspielen."""
    if not interaction.guild:
        await interaction.response.send_message("Nur in Servern nutzbar.", ephemeral=True, silent=True)
        return
    ...
    if not favs:
        await interaction.response.send_message(
            "📭 Keine Favoriten gespeichert.", ephemeral=True, silent=True,
        )
        return
    ...
    if idx < 0 or idx >= len(favs):
        await interaction.response.send_message(
            f"❌ Favorit Nummer **{number}** nicht gefunden. (1-{len(favs)})",
            ephemeral=True, silent=True,
        )
        return
    ...
    queue_item = {
        "url": fav["url"],
        "title": fav.get("title") or fav["url"],
        "webpage_url": fav["url"],
        "is_single": True,
        "requester": interaction.user,
    }
    ...
    await interaction.followup.send(
        f"⭐ Favorit **{number}. {fav.get('title', fav['url'])}** wird abgespielt!",
        silent=True,
    )


# ── /favorite_remove ───────────────────────────────────────────────────────────
@bot.tree.command(name="favorite_remove", description="Einen Favoriten löschen")
@app_commands.describe(
    number="Nummer des Favoriten (aus /favorite_list)",
)
async def favorite_remove(interaction: discord.Interaction, number: int):
    """Favorit per Nummer löschen."""
    if not interaction.guild:
        await interaction.response.send_message("Nur in Servern nutzbar.", ephemeral=True, silent=True)
        return
    ...
    removed = favs[idx]
    ok = remove_favorite(guild_id, removed["name"])
    if ok:
        await interaction.response.send_message(
            f"🗑️ Favorit **{number}. {removed.get('title', removed['url'])}** gelöscht.",
            ephemeral=True, silent=True,
        )
    else:
        await interaction.response.send_message(
            f"❌ Fehler beim Löschen von **{number}. {removed.get('title', '?')}**.",
            ephemeral=True, silent=True,
        )
