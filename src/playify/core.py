...
# ==============================================================================
# PO Token Helper — auto-fetch from bgutil-pot server
# ==============================================================================
...
# bgutil-pot server endpoint (must match systemd service --host/--port)
BGUTIL_POT_URL = os.getenv("BGUTIL_POT_URL", "http://127.0.0.1:4416")

# How long to cache a token (seconds). bgutil-pot tokens expire ~6h, we refresh every 5 min
PO_TOKEN_CACHE_TTL = 300


def get_po_token(video_id: str | None = None) -> str | None:
    """
    Fetch a PO Token from the bgutil-pot HTTP server.
    Tokens are cached for PO_TOKEN_CACHE_TTL seconds to avoid excessive requests.
    Returns None if the server is unreachable.
    """
    now = time.time()
    if _PO_TOKEN_CACHE["token"] and now < _PO_TOKEN_CACHE["expires_at"]:
        return _PO_TOKEN_CACHE["token"]

    # Use a well-known video ID for content binding if none provided
    content_id = video_id or "dQw4w9WgXcQ"

    try:
        data = json.dumps({
            "content_binding": content_id,
            "challenge": None,
        ...
        token = result.get("poToken")
        if token:
            _PO_TOKEN_CACHE["token"] = token
            _PO_TOKEN_CACHE["expires_at"] = now + PO_TOKEN_CACHE_TTL
            logger.info("PO Token refreshed from bgutil-pot server.")
            return token
    except Exception as e:
        logger.warning(f"Failed to fetch PO Token from bgutil-pot: {e}")
...
def _fav_conn():
    """Get a new DB connection for favorites."""
    conn = sqlite3.connect(DB_PATH)
    conn.row_factory = sqlite3.Row
    return conn


def add_favorite(guild_id: int, name: str, url: str, title: str, added_by: int) -> bool:
    """Add a favorite. Returns True on success, False if name already exists."""
    try:
        conn = _fav_conn()
        conn.execute(
            "INSERT INTO favorites (guild_id, name, url, title, added_by, created_at) VALUES (?, ?, ?, ?, ?, ?)",
            (guild_id, name.lower().strip(), url, title, added_by, time.time()),
        )
        conn.commit()
        conn.close()
        return True
...
def remove_favorite(guild_id: int, name: str) -> bool:
    """Remove a favorite by name. Returns True if found and removed."""
    conn = _fav_conn()
    cur = conn.execute("DELETE FROM favorites WHERE guild_id = ? AND name = ?", (guild_id, name.lower().strip()))
    conn.commit()
...
def get_favorite(guild_id: int, name: str) -> dict | None:
    """Get a single favorite by name."""
    conn = _fav_conn()
    row = conn.execute("SELECT * FROM favorites WHERE guild_id = ? AND name = ?", (guild_id, name.lower().strip())).fetchone()
    conn.close()
    return dict(row) if row else None


def list_favorites(guild_id: int) -> list[dict]:
    """List all favorites for a guild."""
    conn = _fav_conn()
    rows = conn.execute("SELECT * FROM favorites WHERE guild_id = ? ORDER BY created_at", (guild_id,)).fetchall()
    conn.close()
...
# Cookie-Dateien werden automatisch aus dem Bot-Root-Verzeichnis geladen
# Einfach cookies.txt (oder cookies_1.txt etc.) in /home/playify/ legen
import glob as _glob
AVAILABLE_COOKIES = sorted(_glob.glob(str(PROJECT_ROOT / "cookies*.txt")))
if not AVAILABLE_COOKIES:
    AVAILABLE_COOKIES = []  # Keine Cookies = kein Fallback für age-restricted
...
class GuildModel:
    ...
    def __init__(self, guild_id: int):
        ...
        self.controller_delete_timer: asyncio.Task | None = None
