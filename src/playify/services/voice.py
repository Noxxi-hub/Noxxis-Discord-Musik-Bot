...
async def fetch_video_info_with_retry(query: str, ydl_opts_override=None):
    ...
    # PO Token wird nur noch für Android genutzt (iOS blockt Server-IPs)
    po_token = get_po_token()
    if po_token:
        base_ydl_opts["extractor_args"]["youtube"]["po_token"] = [f"android.gvs+{po_token}"]
    ...
    try:
        ...
        if (
            ...
            if not AVAILABLE_COOKIES:
                logger.warning(
                    f"Restriction for '{query[:100]}' but no cookies available. "
                    "User needs to provide cookies.txt for age-restricted content."
                )
                raise e

            logger.warning(
                f"Restriction/Bot detection for '{query[:100]}'. Retrying with {len(AVAILABLE_COOKIES)} cookie file(s)."
            )
...
def clear_audio_cache(guild_id: int):
    """Löscht das Audio-Cache-Verzeichnis für eine bestimmte Guild."""
...
def get_full_opts():
    ...
    # PO Token für Android (iOS blockt Server-IPs)
    po_token = get_po_token()
    if po_token:
        opts["extractor_args"]["youtube"]["po_token"] = [f"android.gvs+{po_token}"]
    return opts
